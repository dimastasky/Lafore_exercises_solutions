...
public:
	...
	explicit bMoney(long double s) : sum(s) // конструктор с одним параметром (числовое значение суммы)
		{ }
	...
	{
		return bMoney(sum + m2.sum);
	}
	bMoney operator-(bMoney m2) const
	{
		return bMoney(sum - m2.sum);
	}
	//Цена за единицу времени, затраченного на изделие
	bMoney operator*(long double t_unit) const
	{
		return bMoney(sum * t_unit);
	}
	//Общая цена, деленная на цену за изделие
	long double operator/ (bMoney price) const
	{
		return long double(sum / price.sum);
	}
	//Общая цена, деленная на количество изделий
	bMoney operator/ (long double quntity) const
	{
		return bMoney(sum / quntity);
	}
...
int main()
{
	...
	wchar_t yesno;			 // для ответа пользователя о продолжении (y/n)
	...
	do
	{
		...
		wcout << L"Попробовать с другими исходными данными (y/n)? "; wcin >> yesno; wcout << endl;
	} while (yesno != 'n');
